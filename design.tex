...
Both devices pick random session keys, call them $s_1$ and $s_2$. Both clients
...
or $s_1$ on device $D_2$. In both cases, the message flow is as follows, but 
only in the first case is the first message (``start'') required:
...
\begin{tikzpicture}[>=stealth]
  ...
  \draw[->] ($(d1)+(0,-3)$) -- node[above] {please sign: link $l$} ($(d2)+(0,-3)$);
