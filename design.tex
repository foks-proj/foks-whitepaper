...
value that was generated when the $i-1$st link was published. The $i-1$st link
...
When a client requests a sighchain for party $p$, the server returns $i+1$ paths
down from the root of the tree, if the sigchain is $i$ links long. The first 
$i$ point to leafs as described above, and the last is a proof that the $i+1$st 
link does not exist. For each path, the server returns neighbors necessary
to trace the path back to the tree root. The server also returns the sequence
of $r_i$s, so the client can verify they match the commitments in the sigchain.
Note the first link location is not randomized with $r_1$, since there was
no $0$th link to commit to $r_1$. However, this link is already essentially
random since the party's ID $p$ will appear random at the first link 
(and then is predictable thereafter).
